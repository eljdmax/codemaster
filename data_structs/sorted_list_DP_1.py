...
class SortedList(MutableSequence):
	 ...
	 def index(self, val, start=None, stop=None):
		  """
		  Return the smallest *k* such that lines[k] == val and i <= k < j`.  Raises
...
class SortedListWithKey(SortedList):
	 ...
	 def add(self, val, nodes):
		  """Add the element *val* to the list."""
		  _lists = self._lists
		  _keys = self._keys
		  _maxes = self._maxes

		  key = self._key(val,nodes)
	 ...
	 def update(self, iterable, nodes):
		  """Update the list by adding all elements from *iterable*."""
		  _lists = self._lists
		  _keys = self._keys
		  _maxes = self._maxes
	 ...
	 #def __contains__(self, val, nodes):
	 def findValue(self, val, nodes):
		  """Return True if and only if *val* is an element in the list."""
		  _maxes = self._maxes
	 ...
	 def __setitem__(self, index, value, nodes):
		  """
	 ...
	 def bisect_left(self, val, nodes):
		  """
		  Similar to the *bisect* module in the standard library, this returns an
		  appropriate index to insert *val*. If *val* is already present, the
		  insertion point will be before (to the left of) any existing entries.
		  """
		  return self._bisect_key_left(self._key(val, nodes))

	 def bisect_right(self, val, nodes):
		  """
		  Same as *bisect_left*, but if *val* is already present, the insertion
		  point will be after (to the right of) any existing entries.
		  """
		  return self._bisect_key_right(self._key(val,nodes))
	 ...
	 def index(self, val, nodes, start=None, stop=None):
		  """
		  Return the smallest *k* such that lines[k] == val and i <= k < j`.  Raises
		  ValueError if *val* is not present.  *stop* defaults to the end of the
		  list. *start* defaults to the beginning. Negative indices are supported,
...
def intersection( x, y, nodes):
	return  ( 1.0*(nodes[y][1] - nodes[x][1]) ) / (nodes[x][0] - nodes[y][0]) 
				

def lower_bound(lines, nodes, target):
	l, r = 0, len(lines) - 1
	while l <= r:
		mid = l + (r - l) / 2
		if nodes[ lines[mid] ][-1] > target:
			r = mid - 1
		else:
			l = mid + 1
	return l
...
if __name__ == "__main__":
	...
	nodes = [(0.2,1,float('-inf')), (0.2 , 7, float('-inf'))]#, (-3,106, float('-inf')), (-0.2,3, float('-inf')) ,  (0.1, 7, float('-inf') ), (0, -3, float('-inf') ) ]
	
	lines = SortedListWithKey(key=lambda val, nodes: -1*nodes[val][0]* (base**1) - nodes[val][1] )

	
	for k in range( len(nodes)):
		lines.add(k, nodes)
		cur = lines.index(k,nodes)
		print (nodes[k], cur)
		#disgarding cur ?
		if (cur < len(lines)-1):
			if ( nodes[lines[cur]][0]  == nodes[lines[cur+1]][0] ) :
				lines.discard(lines[cur], nodes)
				continue
			if (cur > 0) and (intersection( lines[cur-1], lines[cur+1], nodes) < intersection(lines[cur-1], lines[cur], nodes) ):
				lines.discard(lines[cur], nodes)
				continue
		...
		#disgarding left
		pre =  cur -1
		if (pre >= 0) and ( nodes[lines[pre]][0]  == nodes[lines[pre+1]][0] ) :
			lines.discard(lines[pre], nodes)
			pre -= 1
				
		while (pre > 0):
			n_pre = pre - 1
			if (intersection( lines[n_pre], lines[cur], nodes) >= intersection(lines[n_pre], lines[pre], nodes) ):
				break
			lines.discard(lines[pre], nodes)
			cur -= 1
			pre = n_pre
		...
		top = len( lines ) - 1
		next = cur + 1
		if (next < top) and ( nodes[lines[next]][0]  == nodes[lines[next+1]][0] ) :
			lines.discard(lines[next], nodes)
			top -= 1
		
		while ( next < top):
			n_next = next + 1
			if (intersection( lines[cur], lines[n_next], nodes) >= intersection(lines[cur], lines[next], nodes) ):
				break
			lines.discard(lines[next], nodes)
			top -= 1

		if (cur>0):
			x = intersection( lines[cur-1], lines[cur], nodes)
			nodes[lines[cur]] = nodes[lines[cur]][:-1] + (x,)
			
		if (cur < top):
			x = intersection( lines[cur], lines[cur+1], nodes)
			nodes[lines[cur+1]] = nodes[lines[cur+1]][:-1] + (x,)
			
	
	
	#print(lines.findValue(len(nodes)-1,nodes))
	
	print(','.join(map(str, [nodes[x] for x in lines] )))
	
	print( lower_bound(lines, nodes, 6.7) - 1)
